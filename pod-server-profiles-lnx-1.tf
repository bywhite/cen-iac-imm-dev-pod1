...
module "server_profiles_lnx_1" {                                   # <<-- Change to duplicate
  source = "github.com/bywhite/cen-iac-imm-dev-pod1-mods//imm-pod-srv-profiles"  #?ref=v1.2.5"
...
# -----------------------------------------------------------------------------
  ...
  server_policy_prefix = "${local.pod_policy_prefix}-lnx-1"         # <<-- Change to duplicate
  ...
  tags = [
    ...
    { "key" : "ServerGroup", "value" : "${local.pod_policy_prefix}-lnx-1" } # <-- Change to duplicate
  ]
